...
public class MostRecentPatientCreated extends BaseTestClass {
    public String getMostRecentPatientCreated() throws Exception{ //valueOne
        ...
        utils.query = "SELECT TOP 1 C.FirstName FROM [summa].[dbo].[ServiceRequests] as SR " +
                "JOIN [Summa].[dbo].[ContactAddresses] as CA " +
                "ON SR.ContactAddressID = CA.ContactAddressID " +
                "AND CA.Deleted = 0 AND " +
                "CA.Rank = 1 " +
                "LEFT JOIN [Summa].[dbo].[Contacts] as C " +
                "ON SR.ContactID = C.ContactID " +
                "Where C.ProgramID =250 " +
        ...
        try {
            ...
            String url = "jdbc:sqlserver://192.168.102.119;databaseName=SummaVoyager-UATEnv";
