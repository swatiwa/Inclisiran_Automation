...
public class CreatePatientExcelData extends BaseTestClass {
    ...
    public void CreatePatient() throws Exception{
        ...
        logger.info("Clicking on HCP Portal");
        ...
        context.CreateNewContact(
                ...
                excelReader.testData.get("ContactType"),
...
//          context.CreateNewContact("Sam", "Bandara", "Provider");
        ...
        context.createNewPatient(
                excelReader.testData.get("PatientFname"),
                excelReader.testData.get("PatientLname"));
        String npi = new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()).substring(4);
        context.createNewProvider(
                excelReader.testData.get("ProviderFName"),
                excelReader.testData.get("ProviderLName"),
                npi);
        context.createNewFacility(
                excelReader.testData.get("FacilityName"),
                excelReader.testData.get("FacilityType"));
...
//
        ...
        ///
        List<String> resolutions = Arrays.asList("Complete Intake Form Received", "Complete PAP Intake Received",
                "Cancelled-missing information not received", "Cancelled-Provider non-responsive", "SR created in error");

        for(String temp: resolutions){
            keyElements.selectResolution(temp);
        }

        keyElements.selectResolution("Cancelled-Provider non-responsive");
        keyElements.clickSaveButton();
